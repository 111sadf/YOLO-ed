...
from .EfficientMod import BasicBlock
...
class C2f_Eff(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(BasicBlock(self.c,drop_path=0.2) for _ in range(n))

    def forward(self, x):
        """Forward pass through C2f layer."""
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1].permute(0, 2, 3, 1)).permute(0, 3, 1, 2) for m in self.m)
        return self.cv2(torch.cat(y, 1))
